...
To set up an AI activity for student teams, all you need is to specify the team membership in the "Group" field under the "Students" tab in your instructor portal. Click "Enable Groups" to make this field visible. You can either enter group names along with student names and email into the same csv roster file and upload it, or you can manually enter / modify group names later. Please note that we currently support one group set for each course. 
